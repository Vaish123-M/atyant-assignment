...
export default function Hero() {
  return (
    <section id="top" className="relative overflow-hidden bg-[#0B0F2E] text-white">
      ...
      <div className="relative mx-auto grid w-full max-w-7xl gap-12 px-4 py-20 sm:px-6 lg:grid-cols-2 lg:px-8 lg:py-24">
        ...
        <motion.div initial={{ opacity: 0, y: 20, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.8, delay: 0.1 }} className="relative">
          <div className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-white/8 p-5 shadow-[0_30px_80px_rgba(0,0,0,0.35)] backdrop-blur-xl sm:p-7">
            ...
            <div className="relative grid gap-4 sm:grid-cols-5">
              <div className="sm:col-span-3 rounded-[1.6rem] border border-white/10 bg-[#101738]/80 p-5">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-sm text-white/55">Decision roadmap</div>
                    <div className="mt-1 text-2xl font-black">Confusion to clarity</div>
                  </div>
                  ...
                  {heroDecisionRows.map((item) => (
                    <div key={item.label} className="rounded-2xl bg-white/6 px-4 py-3">
                      <div className="flex items-center justify-between text-sm text-white/78">
                        ...
                        <span className="font-semibold text-[#FFB38E]">{item.value}</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="sm:col-span-2 flex flex-col gap-4">
                <div className="rounded-[1.6rem] border border-white/10 bg-white/8 p-5">
                  ...
                  <div className="mt-2 text-lg font-bold">Rank 80k - 3 lakh</div>
                  <div className="mt-3 flex items-center gap-2 text-sm text-white/72">
                    <CheckCircle2 className="h-4 w-4 text-[#FF6B2B]" />
                    Tier 2 / Tier 3 city
                  </div>
                  {heroProfilePoints.slice(2).map((point) => (
                    <div key={point} className="mt-2 flex items-center gap-2 text-sm text-white/72">
                      <CheckCircle2 className="h-4 w-4 text-[#FF6B2B]" />
                      {point}
                    </div>
                  ))}
                ...
                <div className="rounded-[1.6rem] border border-white/10 bg-white/8 p-5">
                  ...
                  <div className="mt-2 text-lg font-bold">One clear next step</div>
                  <div className="mt-3 inline-flex items-center gap-2 rounded-full bg-[#FF6B2B] px-4 py-2 text-sm font-semibold text-white">
            ...
            <div className="absolute -right-8 top-10 hidden rounded-2xl border border-white/10 bg-white px-4 py-3 text-sm font-semibold text-[#0B0F2E] shadow-xl lg:block">
              4 years saved from regret
            </div>
            <div className="absolute -left-6 bottom-12 hidden rounded-2xl border border-white/10 bg-[#FF6B2B] px-4 py-3 text-sm font-semibold text-white shadow-xl lg:block">
